#!/usr/local/bin/node
​
/* #### WARNING: This script will automatically postback all URLs it gathers to Kao's API!!! ####
If you want to safely preview the postback URLs first, use the _NoPostback version of this script or disable the postbackKaoURLs() function
/*
​
/* 
Lead Payouts With Offset and YES Postback script
​
What it does: 
...
- Loops through the array of Kao URLs and uses async GET requests for postback to Kao
- Returns a key/value object containing each URL that was posted back and its corresponding response status code
- Exits with success code 0 if postback funciton returned
- Exits with error code 1 in case of any error being caught
​
Usage: 
$ node {script name} {number of hours to offset}
e.g. $ node LeadPayoutsOffset_YesPostback.js 6
​
Be Aware:
- this script will automatically postback all URLs it gathers to Kao's API! 
- If you want to safely preview the postback URLs first, use the _NoPostback version of this script or disable the postbackKaoURLs() function
...
if (process.argv.length !== 3) {
  console.log("Incorrect number of arguments. Format should be: $ node {script.js} {argument}");
...
function Main(){
  ...
  .then((leadPayoutsPayload) => { // anonymous function for logging pre-parse output
  ...
  .then(createKaoURLs)
  .then((kaoURLsArray) => { // anonymous function for logging post-parse output
    ...
    console.log('ATTEMPT POSTBACK...........................');
    return kaoURLsArray; // so next function in .then() chain gets data
  })
  .then(postbackKaoURLs)
  .then((requestsResponsesLog) => { // anonymous function for logging post-postback output
    ...
    console.log(logDate, `Number of Postbacks: ${requestsResponsesLog.length}`);
    console.log('........................................END');
  })
​
...
Loops through the array of Kao URLs and uses async GET requests for postback to Kao
​
Returns: a key/value object containing each URL that was posted back (kaoURL) and its corresponding response status code (statusCode),
or an empty list if no lead payouts were found by the request
*/
​
async function postbackKaoURLs(kaoURLsArray) {
​
  let requestsResponsesLog = [];
​
  for (let i = 0; i < kaoURLsArray.length; i++) {
        let postbackResponse = await axios.get(kaoURLsArray[i]);
        let loggedResponse = {kaoURL: kaoURLsArray[i], statusCode: postbackResponse.status};
