...
if (process.argv.length !== 3) {
  ...
  console.log(
    "Example: $ node LeadPayoutsBackfill_NoPostback.js 2021-04-03T20:00:00Z"
  );
  process.exitCode = 1;
  return;
}
...
async function Main() {
  ...
  while (leadPayoutsURL) {
    try {
      ...
      console.log(
        `Length of KaoURLsBundle: ${fullKaoURLsBundle.kaoURLsArray.length}`
      );
      ...
      if (error.response.status === 422) {
        // 422 status code errors handled specially for script functionality
        ...
        if (detailType === "malformed") {
          console.log(
            "########## Reached end of available data. This is expected behavior. ##########"
          );
          console.log(
            "(API response data produced below in case it is needed)"
          );
          ...
          console.log(
            "########## Timestamp is missing or format is incorrect. Example format: 2021-04-03T20:00:00Z ##########"
          );
          console.log(
            "(API response data produced below in case it is needed)"
          );
...
async function filterLeadPayouts(leadPayoutsURL) {
  ...
  if (skippedLeadEvents.length > 0) {
    // if any Lead events are skipped, it will be logged in the console.
    console.log(
      `${skippedLeadEvents.length} Lead Events Skipped due to Invalid Client ID`,
      skippedLeadEvents
    );
  }
